...
describe(RULE_NAME, () => {
  it('stays report-only', () => {
    // Rewriting an import + local declaration into an `export…from` is not safely automatable
    // (JSDoc, extra references, identity wrappers), so the rule intentionally ships no fixer.
